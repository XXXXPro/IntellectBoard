...
class Library_tags extends Library {
  ...
  function set_tags($tag_string,$item,$type=0) {
    ...
    if (!empty($tag_string)) {
      ...
      $taghash = $this->app()->db->select_simple_hash($sql);
      ...
      for ($i=0, $count=count($tags);$i<$count;$i++) {
        if (!isset($taghash[$lowerfunc($tags[$i])])) {
          ...
          $taghash[$tags[$i]]=$this->app()->db->insert_id();
        }
      }
    }
    else $taghash = array();
    ...
    $oldtags = $this->app()->db->select_all_numbers($sql);
  ...
  function get_tags($item,$type=0) {
    ...
    'ORDER BY tagname';
    $tags = $this->app()->db->select_all_strings($sql);
    return $tags;
  }
  ...
  function get_tags_by_ids($items,$type=0) {
    ...
    'WHERE t.id=ti.tag_id AND '.$this->app()->db->array_to_sql($items,'ti.item_id').' AND t.type=\''.intval($type).'\' '.
    'ORDER BY tagname';
    $tags = $this->app()->db->select_all($sql);
    $result=array();
  ...
  function get_all_tags($type=0,$limit=false) {
    ...
    $tags = $this->app()->db->select_all($sql,$limit);
    return $tags;
  }
  ...
  function get_ids_by_tags($tags,$type=0) {
    ...
    $sql = 'SELECT item_id FROM '.DB_prefix.'tagname tn, '.DB_prefix.'tagentry te WHERE '.$this->app()->db->array_to_sql($tags,'tagname').
