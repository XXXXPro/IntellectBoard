# Intellect Board Pro 3.05 Rebuild (версия от 31 января 2026 года)
...
**Примечание**: если вы настроили DocumentRoot на подкаталог www, но при этом получаете ошибку 500, удалите файл .htaccess в корне сайта. Последние версии Apache запрещают размещать файлы .htaccess в каталогах выше DocumentRoot.
...
Для работы Intellect Board достаточно минимального набора прав доступа к СУБД: ```SELECT, INSERT, UPDATE, DELETE, LOCK```. Однако в процессе установки нужны также права на создание и изменение таблиц (CREATE и ALTER). С учетом этого в Intellect Board предусмотрена возможность указать для проведения установки IntB учетные данные более привилегированного пользователя, которые будут использоваться только в момент установки. Для этого нужно указать соответствующую опцию на третьем шаге установки. Кроме того, в этом случае база данных и непривилегированный пользователь могут быть созданы прямо в процессе установки.
