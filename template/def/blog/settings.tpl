...
<div><label><span>Выводить на главной странице ссылки на </span>{{ macros.input('extdata[mainpage_posts]',extdata.mainpage_posts,4) }} сообщений</label></div>
...
</fieldset><fieldset><legend>Уведомления в Telegram</legend>
<div><span>Режим отправки уведомлений</span>{{ macros.select('extdata[telegram_mode]',extdata.telegram_mode,{'0':'Выключены','1':'Краткие уведомления о новых статьях','2':'Уведомления о статьях с полным текстом','3':'Краткие уведомления о всех сообщениях','4':'Уведомления с текстом обо всех сообщениях'}) }}</div> 
...
<div><label><span>Токен</span>{{ macros.input('extdata[vk_token]',extdata.vk_token,48,255,'id="vk_token"') }} {% if forumdata.hurl %}<button onclick="window.open('{{ url(forumdata.hurl) }}/vk_token.htm')" type="button">Получить</button>{% else %}Чтобы получить токен, завершите создание раздела{% endif %}</label></div>
</fieldset>
