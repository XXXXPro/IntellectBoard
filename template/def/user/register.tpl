...
<form action="" method="post" class="ibform" id="user_register">
	<fieldset><legend>Регистрация пользователя</legend>
		<div><label><span>Имя пользователя</span>{{ macros.input("basic[login]",formdata.basic.login,32,'required="required" autocomplte="username"') }}</label>
		<span style="width: auto; display: inline; float: none" id="check_result"></span></div>
		<div><label><span>Пароль</span>{{ macros.password("basic[password]",formdata.basic.password,32,'required="required" autocomplte="new-password"') }}</label></div>
		<div><label><span>Подтверждение пароля</span>{{ macros.password("password_confirm",formdata.password_confirm,32,'required="required" autocomplte="new-password"') }}</label></div>
