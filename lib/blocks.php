...
class Library_blocks extends Library {
   ...
   **/
   function block_announce($mode=1) {
     $result=false;
     if ($mode==0 || empty($this->app()->forum['id'])) { // режим 0 -- показ главного объявления на всех страницах
       if ($mode!=2) $result=$this->app()->get_text(0, 1);
     }
     elseif (!empty($this->app()->forum['id'])) { // если мы в разделе
       $result = $this->app()->get_text($this->app()->forum['id'], 1);
       if (trim($result)=='' && $mode!=2) $result = $this->app()->get_text(0, 1); // если нет локального объявления, выводим главное      
     }
   ...
   function block_is_online($users) {
      ...
      $sql = 'SELECT uid, MAX(visit1) FROM '.DB_prefix.'last_visit WHERE '.$this->app()->db->array_to_sql($users,'uid');
      $result = $this->app()->db->select_simple_hash($sql);
      $online_time = $this->app()->get_opt('online_time'); // количество миут, в течение которых пользователь считается присутстующим на форуме
      if (empty($online_time)) $online_time = 5; // значение по умолчанию, если время онлайн не определено
      $result['limit_time'] = $this->app()->time - $online_time*60;
   ...
   function block_pm_unread() {
     if (!$this->app()->get_opt('enable_privmsg') || $this->app()->is_guest()) return false;
     $sql = 'SELECT SUM(unread) AS unread, MAX(last_post_date) AS last '.
            'FROM '.DB_prefix.'privmsg_thread_user thu '.
            'WHERE uid='.intval($this->app()->get_uid());
     $data = $this->app()->db->select_row($sql);
     $this->app()->lastmod = max($this->app()->lastmod, $data['last']);
   ...
   function block_premoderate($params) {
     if (!$this->app()->is_admin()) return false;
     ...
     $premoderate_forums = $this->app()->db->select_all($sql);
     
     foreach ($premoderate_forums as $item) $this->app()->lastmod=max($this->app()->lastmod,$item['post_time']);
     return array('empty',$premoderate_forums);
   }

   /** Количество просмотров раздела **/
   function block_forum_views($forum_id=false) {
     if (!$forum_id && $this->app()->forum) $forum_id=$this->app()->forum['id'];
     if (!$forum_id) return false;
     $sql = 'SELECT views FROM '.DB_prefix.'views WHERE oid='.intval($forum_id).' AND type=\'forum\'';
     $views = $this->app()->db->select_int($sql);
   ...
   function block_tag_list($params='') {
     ...
     $result['tags'] = $this->app()->db->select_simple_hash($sql);
     $result['limit'] = $limit;
  ...
  function block_active_topics($params='') {
    ...
    else $cond['fid'] = $this->app()->get_forum_list('read');
    ...
    $cond['after_time']=$this->app()->time-$days*24*60*60;
    $cond['first']=true;
    $cond['last']=true;
    ...
    if ($tlib) {
      ...
      foreach ($topics as $topic) $this->app()->lastmod=max($this->app()->lastmod,$topic['last_post_date']);
    }
  ...
  **/
  function block_new_topics($params='') {
    ...
    $cond['perpage']=isset($params[0]) ? intval($params[0]) : 3; // по умолчанию выводим три темы
    $days = !empty($params[1]) ? intval($params[1]) : 30; // по умолчанию выводим за месяц
    $cond['create_time']=$this->app()->time-$days*24*60*60;
    $cond['first']=true;
    $cond['forums']=true; // нужны и данные о разделах
    $cond['not_flood']=true; // не выводим темы из флуд-разделов
    ...
    if ($tlib) {
      $topics = $tlib->list_topics($cond);
      if (empty($topics)) return false;
      foreach ($topics as $topic) $this->app()->lastmod=max($this->app()->lastmod,$topic['first_post_date']);
    }
