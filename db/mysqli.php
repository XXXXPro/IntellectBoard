...
**/
  function _query($sql,$params=false) {
    while (preg_match('|^(\s*SELECT\s+.*?\W)CAST\((.*?)\s+AS\s+integer\)(.*?\s+FROM)|i',$sql)) {
      $sql = preg_replace('|^(\s*SELECT\s+.*?\W)CAST\((.*?)\s+AS\s+integer\)(.*?\s+FROM)|i','$1CAST($2 AS unsigned integer)$3',$sql);    
    }
    if (!empty($params)) {
