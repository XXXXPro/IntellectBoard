...
class Library_topic extends Library {
  ...
  function get_topic($topic_url,$status=0,$user=false) {
      $user=$user && !$this->app()->is_guest();
      $params = array();
      $sql = 'SELECT t.id, t.fid, t.title, t.descr, CONCAT(\''.$this->app()->forum['hurl'].'/\',CASE WHEN hurl!=\'\' THEN hurl ELSE CAST(t.id AS CHAR(11)) END,\'/\') AS full_hurl, hurl, '.
      ...
      $params[] = $status;
      return $this->app()->db->select_row($sql,$params);
  }
  ...
  **/
  function list_topics($cond) {
    ...
    if (!empty($cond['fid'])) {
      if (is_array($cond['fid'])) $where.=' AND '.$this->app()->db->array_to_sql($cond['fid'],'t.fid');
      else $where.=' AND t.fid='.intval($cond['fid']);
    }
    if (!empty($cond['id'])) {
      if (is_array($cond['id'])) $where.=' AND '.$this->app()->db->array_to_sql($cond['id'],'t.id');
    ...
    if (!empty($cond['forums']) && !empty($cond['forumtype'])) {
      ...
      $where.=' AND '.$this->app()->db->array_to_sql($cond['forumtype'],'module');
    }
...
//    if (!empty($cond['first']) && !empty($cond['first_user'])) $sql.='LEFT JOIN '.DB_prefix.'user u2 ON (u2.id=p2.author) ';
  ...
  **/
  function count_topics($cond) {
    ...
    if (!empty($cond['fid'])) {
      $where.=' AND '.$this->app()->db->array_to_sql($cond['fid'],'t.fid');
    }
  ...
  **/
  function get_posts($cond) {
    ...
    if (!empty($cond['fid'])) $where.=' AND '.$this->app()->db->array_to_sql($cond['fid'],'t.fid');
    if (!empty($cond['tid'])) $where.=' AND p.tid='.intval($cond['tid']);
    $uid = $this->app()->get_uid();

    if (!empty($cond['id'])) {
       $where.=' AND '.$this->app()->db->array_to_sql($cond['id'],'p.id');
    }
    ...
    if (!empty($cond['attach'])) { // если запрошено получение прикрепленных файлов
      ...
      $sql = 'SELECT TRIM(fkey) AS fkey, oid, filename, size, format, is_main, descr, exif, geo_longtitude, geo_latitude FROM '.DB_prefix.'file '.
        'WHERE '.$this->app()->db->array_to_sql($pids,'oid').' AND type=\'1\'';
      $attaches = $this->app()->db->select_super_hash($sql,'oid');
  ...
  **/
  function count_posts($cond) {
    ...
    if (!empty($cond['id'])) {
      if (is_array($cond['id'])) $where.=' AND '.$this->app()->db->array_to_sql($cond['id'],'id');
  ...
  function get_first_posts($topics,$options = array()) {
    ...
    for ($i=0,$count=count($topics);$i<$count;$i++) {
      ...
      $topics[$i]['del_key'] = $this->app()->gen_auth_key(false,'delete_topic',$this->app()->url('moderate/'.$this->app()->forum['hurl'].'/'.$topics[$i]['t_hurl']));
    }
    return $topics;
  }
  ...
  **/
  function get_poll($tid) {
    ...
    if (!empty($result)) {
      ...
      $result['variants'] = $this->app()->db->select_all($sql);
      $result['summ'] = 0;
      $result['max'] = 0;
      ...
      $result['closed'] = $result['endtime'] && $result['endtime']<$this->app()->time;
    }
    return $result;
  }
  ...
  **/
  function get_permissions() {
    $result['html']=$this->app()->check_access('html');
    $result['bcode']=$this->app()->forum['bcode'];
    $result['smiles']=$this->app()->forum['max_smiles'];
    $result['attach']=$this->app()->check_access('attach') && ($this->app()->forum['max_attach']>0);
    $result['links']=$this->app()->get_opt('links_mode','group');
    $result['topic']=$this->app()->check_access('topic') && empty($this->app()->forum['locked']); // темы можно создавать, если есть права и форум не закрыт
    $result['post']=$this->app()->check_access('post') && ((empty($this->app()->forum['locked']) && empty($this->app()->topic['locked']) || $this->app()->is_moderator())); // сообщения можно писать, если есть права и не закрыт ни форум, ни тема
    $result['poll']=$this->app()->check_access('poll');
    $result['sticky']=$this->app()->is_moderator();
    $result['lock']=$this->app()->is_moderator();
    $result['favorites']=$this->app()->is_moderator();
    $result['delete']=$this->app()->is_moderator();
    $result['value']=$this->app()->is_moderator();
    $result['postdate']=$this->app()->is_moderator(); // разрешение изменять дату сообщения
    $result['tags']=$this->app()->forum['tags']==0 ? false : ($this->app()->forum['tags']==2 ? $this->app()->is_moderator() : true ); // если режим тегов равен 2, то их исползовать могут только модераторы, если 1 — все
    if (!$this->app()->forum['sticky_post']==1)  $result['sticky_post']=$this->app()->is_moderator();
    elseif ($this->app()->forum['sticky_post']==2) $result['sticky_post']=true;
    else $result['sticky_post']=false;
    $result['ip']=$this->app()->is_moderator(false); // IP-адреса могут видеть настоящие модераторы, но не кураторы
    return $result;
  }
  ...
  function check_unique_hurl($topic) {
    $sql = 'SELECT COUNT(*) FROM '.DB_prefix.'topic t WHERE hurl=\''.$this->app()->db->slashes($topic['hurl']).'\'';
