...
<p>Вы получили это сообщение потому что являетесь зарегистрированным пользователем форума &laquo;{{ get_opt('site_title') }}&raquo;</p>
<p>
{% if user.type=='forum' %}
<a href="{{ http(url('bookmark/unsubscr.htm?unsubscribe_topic='~user.oid~'&authkey='~unsubscribe_key)) }}">Внести эту тему в список игнорируемых, чтобы больше не получать уведомлений о ней</a>
{% endif %}
{% if user.type=='forum' and user.oid!=0 %}
и подписались в настройках на получение уведомлений о сообщениях во всем разделе. <br />
<a href="{{ http(url('bookmark/unsubscr.htm?unsubscribe_forum='~user.oid~'&authkey='~unsubscribe_key)) }}">Отписаться от уведомлений о новых сообщениях в разделе</a>
{% elseif user.type=='forum' and user.oid==0 %}
и подписались в настройках на получение уведомлений о всех сообщениях на форуме. <br />
<a href="{{ http(url('bookmark/unsubscr.htm?unsubscribe_forum=0&authkey='~unsubscribe_key)) }}'">Отписаться от уведомлений о новых сообщениях на всем форуме</a>
{% endif %}
</p>
