...
== Обновление версии 3.05 до 3.05 Rebuild 2

Каких-либо изменений в структуре базы данных по сравнению с исходной 3.05 не было, поэтому необходимость запускать install.php отсутствует. Однако добавился ряд новых путей, поэтому скопируйте etc/routes.def в etc/routes.txt или, если вы модифицировали routes.txt самостоятельно, добавьте туда новые пути. Также проверьте настройки в Центре Администрирования. 

Обратите внимание: поддержка PWA требует дополнительных настроек (короткого названия приложения и путей к значкам), поэтому по умолчанию выключена.