...
class Library_userlib extends Library {
  ...
  function filter_user($data) {
    return $this->app()->filter($data,Library_userlib::$user_fields);
  }

  function filter_settings($data) {
    return $this->app()->filter($data,Library_userlib::$settings_fields);
  }
  ...
  function check_password($uid,$password) {
    $userdata=$this->app()->load_user($uid);
    $crypted_password=$this->app()->crypt_password($password,$userdata['pass_crypt']);
  ...
  function do_login($login,$password,$long=false) {
    ...
    if (!$user_id) { // если из внешней библиотеки не пришло user_id
      $userdata=$this->app()->load_user(0,3,$login); // 3 -- загружаем данные о пользователе по его имени, а не по uid
      if ($userdata['id']<=AUTH_SYSTEM_USERS) return false;
      $crypted_password=$this->app()->crypt_password($password,$userdata['pass_crypt'],$userdata['rnd']);
    }
    else {
      if ($user_id<=AUTH_SYSTEM_USERS) return false;
      $userdata=$this->app()->load_user($user_id,1); // 1 -- загружаем данные о пользователе по его uid
      $crypted_password=$userdata['password']='*';
    }

    if ($userdata['password']!=$crypted_password) {
      if ($this->app()->get_opt('userlib_logs')>4) $this->app()->log_entry ('user', 7, 'userlib.php', 'Неудачная попытка входа пользователя '.$login);
      return false; //если шифрованный пароль не совпал с хранящимся в базе, выходим из процедуры
    }
    $this->app()->set_user($userdata,$long);
    $_SESSION['starttime']=$this->app()->time;
    if ($this->app()->get_opt('userlib_logs')>3) $this->app()->log_entry ('user', 8, 'userlib.php', 'Пользователь '.$login.' вошел на форум.');
  ...
  function force_login($uid) {
    $this->app()->session();
    $udata = $this->app()->load_user($uid,1);
  ...
  function register_user(&$data,$settings,$contacts=false,$social=false,$novalidate=false) {
    ...
    if (empty($errors)) { // если при валидации пользователя не произошло ошибок
      ...
      $def_settings = $this->app()->load_user(3, 2);
      unset($def_settings['basic']['id']);
      $data = $data+$def_settings['basic'];

//      @eval($this->app()->event('userlib_before_register')); // Вызываем дополнительные обработчики

      $data = $this->filter_user($data);
      $this->app()->db->insert(DB_prefix.'user', $data);
      $data['id']=$this->app()->db->insert_id();

      if ($data['id']) { // если пользователя удалось зарегистрировать и ему дали идентификатор
//        @eval($this->app()->event('userlib_after_register')); // Вызываем дополнительные обработчики
        ...
        $this->app()->db->insert(DB_prefix.'user_settings', $settings);

        $ext_data['id']=$data['id']; // создаем запись в таблице расширенных данных
        $ext_data['group_id']=$def_settings['ext_data']['group_id'];
        $ext_data['reg_date']=$this->app()->time;
        $ext_data['reg_ip']=$this->app()->get_ip();
        $this->app()->db->insert(DB_prefix.'user_ext',$ext_data);

        // помечаем все сообщения форума как прочитанные, чтобы если пользователь зайдет в "непрочитанные"
        $mark_data['uid']=$data['id'];
        $mark_data['fid']='0';
        $mark_data['mark_time']=$this->app()->time;
        $this->app()->db->insert(DB_prefix.'mark_all',$mark_data);
        ...
        if (!empty($contacts)) { // если при регистрации были указаны какие-то контакты, записываем их в базу
          ...
          for ($i=1,$count=count($contacts);$i<=$count;$i++) {
            ...
            if ($cdata['cid']) $this->app()->db->insert(DB_prefix.'user_contact', $cdata);
          }
        }
        ...
        $mdata['regdata'] = $data;
        if ($this->app()->get_opt('userlib_logs')>0) $this->app()->log_entry('user', 1, 'userlib.php', 'Пользователь '.$data['login'].' зарегистрирован'); // логгируем событие, если логгирование включено
        if ($activate_mode==1 && !$novalidate) { // если включена активация пользователя администратором и авторизуемся не через соцсеть с проверенными EMail
          $userkey = $this->app()->gen_auth_key($data['id'],'activate'); // эти данные будут использоваться в письме
          $mdata['keylink'] = $this->app()->http($this->app()->url('user/activate.htm?authkey='.$userkey));
          $mdata['uncrypt_password']=$uncrypt_password;
          $this->app()->mail(array('to'=>$data['email'],'subj'=>'Подтверждение регистрации на сайте',
    ...
    else $this->app()->message($errors); // иначе -- выводим сообщения об ошибке
  ...
  function update_user($data,$settings,$contacts=false,$interests=false,$force=false) {
    if ($this->app()->get_uid()!=$data['id'] && $force==false) { // если не включен режим изменения чужого профиля, то идентфиикатор редактируемого пользователя должен совпадать с залогиненным
      $this->app()->message('Нельзя редактировать чужой профиль!',3);
      return false;
    }

    $olddata = $this->app()->load_user($data['id']);
    $change_email=($olddata['email']!=$data['email']);
    $change_login=($olddata['login']!=$data['login']);

    $log_level = $this->app()->get_opt('userlib_logs'); // уровень логгирования действий над профилем
    ...
    if (isset($data['password']) && $data['password']) { // если новый пароль задан (проверка ввода подтверждения пароля будет делаться в user.php)
      ...
      $data['password']=$this->app()->crypt_password($data['password'], $crypt_mode,$olddata['rnd']); //шифруем пароль
    ...
    if (!$this->app()->get_opt('custom_title','group')) $data['title']=$this->app()->get_opt('title','group'); // если в группе не разрешено задание собственного статуса, пишем в статус название группы
    ...
    if (empty($errors)) { // если при валидации пользователя не произошло ошибок
      ...
      if (isset($data['avatar_delete']) && $data['avatar_delete']) { // если пользователь поставил галочку "удалить аватар", то удаляем, и сбрасываем признак загрузки
        ...
        if ($olddata['avatar']!='none') { // если раньше был аватар и его формат отличается от текущего, удаляем старый файл с аватаром (если совпадает, то необходимости в этом нет, мы его просто перезаписали в функции save_fit_to)
            ...
            if ($this->app()->valid_file($filename)) unlink($filename);
        }
        $data['avatar']='none';
      }
      ...
      if ($up_avatar || $up_photo) { // если загрузили аватар или фото, нужно уменьшить их до нужного размера и положить в каталог
        ...
        if (!$image) $this->app()->message('Не удалось загрузить библиотеку обработки изображений',2);
          ...
          if ($up_avatar) {
            $max_av_x = $this->app()->get_opt('userlib_avatar_x');
            if (!$max_av_x) $max_av_x = 120;
            $max_av_y = $this->app()->get_opt('userlib_avatar_y');
            if (!$max_av_y) $max_av_y = 120;
            $avatar = $image->load($_FILES['avatar']['tmp_name']);
            if (!$avatar) $this->app()->message('Не удалось распознать графический файл аватара',2);
            else {
              ...
              if ($type==IMAGETYPE_JPEG) { // если загрузили JPEG, то обрабатываем его с качеством, заданным в настройках
                $options = $this->app()->get_opt('userlib_avatar_jpeg_qty');
          ...
          if ($up_photo) {
            $max_ph_x = $this->app()->get_opt('userlib_photo_x');
            if (!$max_ph_x) $max_ph_x = 240;
            $max_ph_y = $this->app()->get_opt('userlib_photo_y');
            if (!$max_ph_y) $max_ph_y = 400;
            $photo = $image->load($_FILES['photo']['tmp_name']);
            if (!$photo) $this->app()->message('Не удалось распознать графический файл аватара',2);
            else {
              if ($photo['type']==IMAGETYPE_BMP) $type=IMAGETYPE_PNG;
              else $type=$photo['type'];
              if ($type==IMAGETYPE_JPEG) { // если загрузили JPEG, то обрабатываем его с качеством, заданным в настройках
                $options = $this->app()->get_opt('userlib_photo_jpeg_qty');
...
//      @eval($this->app()->event('userlib_before_update')); // Вызываем дополнительные обработчики

      $data=$this->filter_user($data);
      $result=$this->app()->db->update(DB_prefix.'user', $data, $condition);

      if ($result) { // если изменения в профиль внесены успешно
//        @eval($this->app()->event('userlib_after_update')); // Вызываем дополнительные обработчики
        ...
        if ($need_log) {
          if ($this->app()->get_uid()==$data['id']) $this->app()->log_entry('user', 2, 'userlib.php', 'Пользователь '.$data['login'].' ('.$data['display_name'].') отредактировал свой профиль. '.$log_string);
          else $this->app()->log_entry('user', 2, 'userlib.php', 'Пользователь '.$this->app()->get_userlogin().' отредактировал профиль пользователя '.$data['login'].' ('.$data['display_name'].'). '.$log_string);
        }
        ...
        if ($change_email && $activate_mode==1 && !$force) { // если включена активация пользователя через его ящик EMail, и редактируем не через админку (
          $userkey = $this->app()->gen_auth_key($data['id'],'activate'); // эти данные будут использоваться в письме
          $mdata['keylink'] = $this->app()->http(dirname($_SERVER['REQUEST_URI']).'/activate.htm?authkey='.$userkey);
          $mdata['regdata'] = $data;
          $this->app()->mail(array('to'=>$data['email'],'subj'=>'Подтверждение изменения почтового адреса',
        ...
        if ($this->app()->get_uid()==$data['id'])$this->app()->set_user($this->app()->load_user($data['id'],1)); // загружаем обновленные данные пользователя в сессию
      }
    }
    else $this->app()->message($errors); // иначе -- выводим сообщения об ошибке
  ...
  function update_avatar($uid,$avatar) {
    $sql = 'UPDATE '.DB_prefix.'user SET avatar=\''.$this->app()->db->slashes($avatar).'\' WHERE id='.intval($uid);
  ...
  function validate_user($data,$social=false) {
    ...
    elseif (!$this->valid_email($data['email'])) {
      ...
      if ($this->app()->get_opt('userlib_check_mx') && function_exists('getmxrr')) {
...
/*      if ($this->app()->check_list('user_banned_mail',strtolower($host))) {
...
//    @eval($this->app()->event('userlib_validate')); // Вызов внешних обработчиков, если надо
  ...
  function validate_login($data,$social=false) {
    $result = array();
    $name_mode=$this->app()->get_opt('userlib_name_mode');
    ...
    elseif ($name_mode==5) $regexp=$this->app()->get_opt('userlib_name_regexp');
    if ($regexp=='') $regexp='/^[^,;"\'`]+$/';
  ...
  function get_uid_by_name($name,$canonical=false) {
    ...
    $sql = 'SELECT id FROM '.DB_prefix.'user WHERE '.$fieldname.'=\''.$this->app()->db->slashes($name).'\'';
    $id = $this->app()->db->select_int($sql);
    return $id;
  }
  ...
  function get_uid_by_email($email) {
    $sql = 'SELECT id FROM '.DB_prefix.'user WHERE email=\''.$this->app()->db->slashes($email).'\'';
    $id = $this->app()->db->select_int($sql);
    return $id;
  }
  ...
  function increment_user($uid=false) {
   ...
   if ($uid>AUTH_SYSTEM_USERS) { // гостям и служебным пользователям статус повышаться не должен
    ...
    if ($uid==$this->app()->get_uid() && isset($_SESSION['user'])) { // если увеличили счетчик текущего пользователя и его сессия загружена, берем данные из нее, чтобы не делать лишнего запроса к БД
      ...
      $userdata = $this->app()->load_user($uid,1);
    ...
    if (!$special) { // увеличиваем уровень пользователя только в том случае, если он не состоит ни в какой
      $days = floor(($this->app()->time - $regdate)/(24*60*60)); // высчитываем, сколько дней назад пользователь зарегистрировался
      ...
      $level = $this->app()->db->select_int($sql);
      if ($level) { // если группа найдена
        $sql = 'UPDATE '.DB_prefix.'user_ext SET group_id='.intval($level).' WHERE id='.intval($uid);
        $this->app()->db->query($sql);
        if ($uid==$this->app()->get_uid()) { // если обновили текущего пользователя, то загружаем его новые данные, так как могли поменяться права доступа
          $this->app()->set_user($this->app()->load_user($uid,1));
        }
      }
    }
   }
  ...
  function user_resync($uid) {
    ...
    $count_posts = $this->app()->db->select_int($sql);
  ...
  function get_letters($start=false,$all=false) {
    if ($start) {  $sqline='AND SUBSTR(u.display_name,1,1)=\''.$this->app()->db->slashes($start).'\' ';
      $pos=2;
    }
    else { $sqline ='';  $pos = 1;  }
    ...
    'ORDER BY letter';
    return $this->app()->db->select_all_strings($sql);
  }
  ...
  function list_groups($cond) {
    ...
    return $this->app()->db->select_all($sql);
  }
  ...
  function count_users($cond) {
    ...
    if (!empty($cond['letter'])) $where.=' AND display_name LIKE \''.$this->app()->db->slashes($cond['letter']).'%\'';
    if (!empty($cond['group'])) $where.=' AND g.level='.intval($cond['group']);
    if (!empty($cond['login']))
    if (is_array($cond['login'])) $where.=' AND '.$this->app()->db->array_to_sql($cond['login'],'display_name');
    else $where.=' AND display_name LIKE \''.$this->app()->db->slashes($cond['login']).'%\'';
    if (!empty($cond['location'])) $where.=' AND u.location LIKE \''.$this->app()->db->slashes($cond['location']).'%\'';
    if (!empty($cond['tag'])) $where.=' AND tn.tagname=\''.$this->app()->db->slashes($cond['tag']).'\'';
  ...
  **/
  function list_users($cond) {
    ...
    if (!empty($cond['banned'])) $where.=' AND (u.status=\'2\' OR ue.banned_till>'.intval($this->app()->time).') '; // пользователь может быть изгнан двумя способами: по баллам и безусловно, выбираем его и в том, и в другом случае
    if (!empty($cond['login']))
      if (is_array($cond['login'])) $where.=' AND '.$this->app()->db->array_to_sql($cond['login'],'display_name');
      else $where.=' AND display_name LIKE \''.$this->app()->db->slashes($cond['login']).'%\'';
    if (!empty($cond['friends_list'])) $where .= ' AND (ur1.type=\'friend\' OR ur2.type=\'friend\')';
    if (!empty($cond['letter'])) $where.=' AND display_name LIKE \''.$this->app()->db->slashes($cond['letter']).'%\'';
    if (!empty($cond['group'])) $where.=(is_array($cond['group'])) ? ' AND '.$this->app()->db->array_to_sql($cond['group'],'g.level') : ' AND g.level='.intval($cond['group']);
    if (!empty($cond['location'])) $where.=' AND u.location LIKE \''.$this->app()->db->slashes($cond['location']).'%\'';
    if (!empty($cond['tag'])) $where.=' AND tn.tagname=\''.$this->app()->db->slashes($cond['tag']).'\'';
    ...
    if (!empty($cond['contacts'])) { // подгружаем контактные данные пользователя
      ...
      if (!empty($uids)) {
        ...
        'WHERE '.$this->app()->db->array_to_sql($uids,'uc.uid').' AND uc.cid=uct.cid';
        $contacts = $this->app()->db->select_super_hash($sql,'uid');
  ...
  **/
  function ext_check_access($fids,$gid,$action) {
    ...
    $fids[]=0;    
    if (!isset($cache[$gid])) {
      $sql = 'SELECT fid, "view", "'.$this->app()->db->slashes($action).'" FROM '.DB_prefix.'access WHERE '.$this->app()->db->array_to_sql($fids,'fid').' AND gid='.intval($gid);
      $data = $this->app()->db->select_hash($sql,'fid');
      $cache[$gid]=$data;
    }
    else $data=$cache[$gid];
  ...
  function get_homepage($uid=false) {
    ...
    $hurl = $this->app()->db->select_str($sql,array($uid));
    if (!$hurl) return false;
    else return $this->app()->http($this->app()->url($hurl));
  }
}
